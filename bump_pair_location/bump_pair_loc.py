...
###
    ...
    def find_pair_for_Input(self):
        ...
        for entry in bump_pair_rslt:
            ...
            if self.BumpDataLib_CSV_File_Volt_Minus1_Default != -1 and self.BumpDataInput_CSV_File_Volt_Minus1_Default != -1:
                fout.write(str(entry[0]) + ', ' + str(entry[1]) + ', '+ \
                    str(self.bumpDataInput_bumpCoord[idx_, 0]) + ', ' + str( self.bumpDataInput_bumpCoord[idx_, 1]) + ', '\
        ...
        print('#INFO: ALL '+ str(self.bumpDataInput_bumpCoord.shape[0]) + ' target bumps paired. Quit normally')
